...
export default async function PortalIssueArchivePage({
  ...
  // «Alle avvik» skal faktisk vise alle — de åpne øverst, de utbedrede i
  // månedsmapper under. Tidligere gikk lenken rett til bare de lukkede.
  const [openIssues, folders] = await Promise.all([
    db.issue.findMany({
      where: {
        area: { customerId: user.customerId },
    ...
    listCustomerClosedIssueMonths(user.customerId),
  ]);
  ...
  return (
    <div className="flex animate-rise flex-col gap-7">
      <div className="flex flex-col gap-4">
        ...
        <h1 className="text-display text-ink">Avvik</h1>
      </div>

      <section>
        <h2 className={sectionHeadClass}>
          <span>Åpne nå</span>
          {openIssues.length > 0 ? <span>{openIssues.length}</span> : null}
        </h2>
        <PortalIssueList
          canComment
          ...
          emptyText="Ingen åpne avvik akkurat nå."
        />
      </section>
