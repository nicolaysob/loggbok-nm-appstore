...
- `LogEntry.type` har tre verdier med hver sine regler, håndhevet i applikasjonslaget: `VISIT_NOTE` krever `comment` og har ingen timer, `TASK_COMPLETION` har bare `completedTasks`, `EXTRA_WORK` krever både `hours` og `comment`.
